...
public class Neurons {
	...
	public Neurons(int... NETWORK_LAYER_SIZES) {
	...
	public double[] calculate(double... input) {
		...
		for (int layer = 1; layer < NETWORK_SIZE; layer++) {
			for (int neuron = 0; neuron < NETWORK_LAYER_SIZES[layer]; neuron++) {
				...
				output[layer][neuron] = sigmoidfuc(sum);
...
//		System.out.println("AA"+v / (2d * target.length));
	...
	public void train(double[] input, double[] target, double eta) {
		if (input.length != INPUT_SIZE || target.length != OUTPUT_SIZE)
			return;
		calculate(input);
		backprogation(target);
		updateWeights(eta);
	}

	public void backprogation(double[] target) {
	...
	public double sigmoidfuc(double x) {
//		System.out.println("sigmoid"+x+"-"+1d / (1 + Math.exp(-x)));
